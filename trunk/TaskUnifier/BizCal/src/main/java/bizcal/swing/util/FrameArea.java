...
public class FrameArea extends JComponent implements ComponentListener {
	...
	public FrameArea() {
		/* ================================================== */
		this.normalFont = UIManager.getFont("Label.font").deriveFont(11.0f);
	...
	public void setEvent(Event event) {
		...
		/* ------------------------------------------------------- */
		this.alphaFontColor = new Color(
				this.fontColor.getRed(),
				this.fontColor.getGreen(),
				this.fontColor.getBlue(),
				220);
		try {
	...
	public void addChild(FrameArea fa) {
		...
		if (this.children == null)
			this.children = Collections.synchronizedList(new ArrayList<FrameArea>(
					1));
		/* ------------------------------------------------------- */
	 ...
	 * @param str
	 */
	public synchronized void setMovingTimeString(
			Date moveStartDate,
			Date moveEndDate) {
		/* ================================================== */
		// System.out.println("FrameArea::setMovingTimeString " + moveStartDate
		// + " -" + moveEndDate);
		this.moveDate = (Date) moveStartDate.clone();
		this.movingString = this.timeFormat.format(this.moveDate)
				+ " - "
				+ this.timeFormat.format(moveEndDate);
		/* ================================================== */
	}
	...
	public void paint(Graphics g) {
		...
		if (this.showHeader)
			...
			if (this.event != null) {
				...
				BufferedImage buffImgHeader = new BufferedImage(
						width,
						HEADER_HEIGHT,
						BufferedImage.TYPE_INT_ARGB);
				gbiHeader = buffImgHeader.createGraphics();
				/* ------------------------------------------------------- */
				gbiHeader.setRenderingHint(
						RenderingHints.KEY_ANTIALIASING,
						RenderingHints.VALUE_ANTIALIAS_ON);
				...
				if (this.isRoundedRectangle())
					gbiHeader.fill(new RoundRectangle2D.Double(
							0,
							0,
							width,
							20,
		...
		int xpos = 5;
		if (this.icon != null) {
			g2.drawImage(this.icon.getImage(), xpos, 2, this);
		...
		if (this.event == null || !this.event.isBackground())
			...
			g2.setPaint(this.alphaFontColor);
		/* ------------------------------------------------------- */
		g2.setFont(timeFont);
		int ypos = 15;
		if (this.itsHeadLine != null) {
			g2.drawString(this.itsHeadLine, xpos, ypos);
			ypos += 15;
			xpos = 5;
		}
		...
		if (this.itsDescription != null) {
			...
			int itsW = g2.getFontMetrics().stringWidth(this.itsDescription);
			...
			if (itsW > this.getWidth()) {
				...
				if (this.lineWrap < 0
						|| fm.stringWidth(this.itsDescription.substring(
								0,
								this.lineWrap)) < splitWidth - 15
						|| fm.stringWidth(this.itsDescription.substring(
								0,
								this.lineWrap)) > splitWidth) {
					/* ------------------------------------------------------- */
					// wrap the lines
					...
					while (fm.stringWidth(s) > splitWidth && this.lineWrap > -1) {
				...
				while (pos < this.itsDescription.length() && this.lineWrap > 0) {
					if (pos + this.lineWrap >= this.itsDescription.length())
						g2.drawString(
								this.itsDescription.substring(
										pos,
										this.itsDescription.length() - 1).trim(),
								xpos,
								yposString);
					else
						try {
							g2.drawString(
									this.itsDescription.substring(
											pos,
											pos + this.lineWrap).trim(),
									xpos,
									yposString);
						} catch (Exception e) {
							e.printStackTrace();
						}
					pos = pos + this.lineWrap;
				...
				// just print
				/* ------------------------------------------------------- */
				g2.drawString(this.itsDescription, xpos, ypos);
				/* ------------------------------------------------------- */
			}
			/* ------------------------------------------------------- */
			// }
		...
		if (eTime != null) {
			...
			g2.setFont(timeFont);
			g2.drawString(
					this.timeFormat.format(eTime) + " ",
					xpos + this.getBounds().width - 40,
					ypos + this.getBounds().height - 20);
			/* ------------------------------------------------------- */
		}
		/* ------------------------------------------------------- */
		...
		if (this.startTime != null) {
			...
			g2.setFont(timeFont);
			g2.drawString(
					this.timeFormat.format(this.startTime) + " ",
					xpos,
					ypos);
			/* ------------------------------------------------------- */
		}
		...
		if (this.isMoving) {
			...
			g2.drawString(this.getMovingTimeString(), xpos
					+ this.getBounds().width
					- 85, ypos + this.getBounds().height - 35);
			/* ------------------------------------------------------- */
		}
		...
		if (this.selected && (this.event == null || !this.event.isBackground())) {
			...
			g2.setPaint(this.selectionColor);
	 ...
	 * @version
	 * <br>
	 *          $Log: FrameArea.java,v $ <br>
	 *          Revision 1.13 2008/10/27 13:59:50 heine_ <br>
	 *          fixed invite loop. <br>
	 *          happens when the paint width of a character is bigger than the
	 *          width of the frame area <br>
	 * <br>
	 *          Revision 1.12 2008/10/21 15:08:31 heine_ <br>
	 *          *** empty log message *** <br>
	 * <br>
	 *          Revision 1.11 2008/08/12 12:47:28 heine_ <br>
	 *          fixed some bugs and made code improvements <br>
	 * <br>
	 *          Revision 1.10 2008/06/10 13:16:36 heine_ <br>
	 *          *** empty log message *** <br>
	 * <br>
	 *          Revision 1.9 2008/06/09 14:10:09 heine_ <br>
	 *          *** empty log message *** <br>
	 * <br>
	 *          Revision 1.8 2008/05/30 11:36:47 heine_ <br>
	 *          *** empty log message *** <br>
	 * <br>
	 *          Revision 1.7 2007/09/20 07:23:16 heine_ <br>
	 *          new version commit <br>
	 * <br>
	 *          Revision 1.25 2007-09-18 09:52:33 heinemann <br>
	 *          *** empty log message *** <br>
	 * <br>
	 *          Revision 1.24 2007/08/22 11:58:23 heinemann <br>
	 *          *** empty log message *** <br>
	 * <br>
	 *          Revision 1.23 2007/08/06 11:21:59 heinemann <br>
	 *          *** empty log message *** <br>
	 * <br>
	 *          Revision 1.22 2007/07/18 07:27:19 heinemann <br>
	 *          line wrap for description string <br>
	 * 
	 */
	 ...
	 * @param height
	 * @param round
	 *            round rectangle or not
	 */
	public void drawHatchedRect(
			Graphics g,
			int x,
			int y,
			int width,
	...
	private static int deltaE(int[] lab1, int[] lab2) {
		...
		deltaE = (int) Math.sqrt(pow(deltaL, 2.0)
				+ pow(deltaA, 2.0)
				+ pow(deltaB, 2.0));
